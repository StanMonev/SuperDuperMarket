...
class MeatTest {
   ...
   void setUp() {
      ...
      vacuumPackedMeat = new Meat("MT002", "Lamb Chops", 62, validVacuumPackedExpiryDate, 20.00, MeatTypeEnum.LAMB,
            true);
   }
   ...
   void testInvalidVacuumPackedMeatExpiryDate() {
      LocalDate invalidExpiryDate = LocalDate.now().plusDays(11); // Invalid for vacuum-packed meat (should be 8-10
                                                                  // days)
      Exception exception = assertThrows(IllegalArgumentException.class,
   ...
   void testExpiredMeatQualityDecrease() {
      Meat expiredMeat = new Meat("MT006", "Expired Pork", 55, LocalDate.now().plusDays(3), 10.00, MeatTypeEnum.PORK,
            false);
